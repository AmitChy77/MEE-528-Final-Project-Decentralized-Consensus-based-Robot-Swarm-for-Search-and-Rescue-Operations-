function Model_4()
% Model 4: WITH location sharing, WITH workload balancing
...
% ---- detection + location sharing ----
sense_R       = 0.95;         % detection radius
R_exclude     = 1.00;         % exclusion disk around found persons (m)
k_avoid_found = 2.20;         % avoidance bump gain
sigma_found   = R_exclude/3;  % Gaussian width for avoidance bump
...
% ---- Setup figure ----
figName = 'Model 4: Full System (Location + Workload Sharing) - Animated';
f = figure('Color','w','Name',figName, 'Position', [100 100 1400 600]);
set(f, 'Resize', 'off'); % Prevent figure resizing during animation
...
% ---- Setup animation recording (robust version) ----
video_filename = 'model4_seed20_trial1.avi';
writerObj = [];
try
    % Use the same reliable settings as Model 2
...
for t = 1:T
    ...
    % ---- avoidance of already-found persons (location sharing ON) ----
    found_mask = double(found(:))';      % 1×M
    F_found_avoid = zeros(N,2);
    if any(found_mask)
        for i = 1:N
            dTm  = P(i,:) - Targets;             % M×2
            r2m  = sum(dTm.^2,2);                % M×1
            phi_bump = found_mask(:) .* exp(-r2m/(2*sigma_found^2));
            grad_phi = (dTm / max(sigma_found^2,1e-6)) .* phi_bump;
            F_found_avoid(i,:) = sum(grad_phi,1);
        end
    end
    ...
    U_det = k_lloyd*(Ccent - P) ...
          ...
          + k_consensus*F_cons ...           % workload sharing
          + k_repulse*R ...
          - W ...
          - k_center*(P - center) ...
          + k_avoid_found*F_found_avoid;     % location sharing
    ...
    % ---- update rho: decay near robots + zeroing near found ----
    for i = 1:N
    ...
    if any(found_mask)
        for m = 1:M
            d2p = (xg - Targets(m,1)).^2 + (yg - Targets(m,2)).^2;
            influence = exp(-d2p/(2*(0.90*sigma_found)^2));
            rho = rho .* (1 - 0.90*found_mask(m)*influence);
        end
    end
    ...
    % Targets with exclusion disks for found ones
    for m = 1:M
        ...
        if found(m)
            ...
            plot(ax1, Targets(m,1)+R_exclude*cos(th), ...
                      Targets(m,2)+R_exclude*sin(th), ...
                      ':', 'Color',[0 0.5 0], 'LineWidth',1.0);
        end
    end
    title(ax1, sprintf('Full System (Location + Workload)\n t = %.1f s, found %d/%d', ...
           t*dt, sum(found), M));

    % Right plot with full history
    ...
    % Capture frame for animation (ROBUST VERSION)
    if ~isempty(writerObj) && isvalid(f)
        try
            ...
            if ~isempty(frame.cdata) && all(size(frame.cdata) > 0)
            ...
            % Don't display warning for every frame, only once
            if t == 1 || mod(t, 100) == 0
                fprintf('Video frame capture issue at t=%d: %s\n', t, ME.message);
            end
            % Continue simulation even if video fails
        end
    end
end
...
if ~isempty(writerObj) && isvalid(f)
    try
        for i = 1:round(writerObj.FrameRate * 2)  % ~2 seconds of still frames
            drawnow;
            frame = getframe(f);
            if ~isempty(frame.cdata) && all(size(frame.cdata) > 0)
                writeVideo(writerObj, frame);
        ...
        fprintf('Error writing final still frames: %s\n', ME.message);
    end
end
