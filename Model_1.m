function Model_1()
% SAR Baseline: NO location sharing, NO workload balancing
...
P = [rand(N,1)*diff(box(1,:))+box(1,1), rand(N,1)*diff(box(2,:))+box(2,1)];
...
% ---- Setup figure ----
figName = 'Model 1: Baseline (No Sharing, No Workload) - Animated';
...
% ---- Setup animation recording ----
video_filename = 'model1_seed40.avi';
try
    % use Motion JPEG AVI for robustness
    writerObj = VideoWriter(video_filename, 'Motion JPEG AVI');
    % match frame rate roughly to simulation time
...
t_last = 0;  % last completed iteration index (for safety)
...
for t = 1:T
    ...
    for i = 1:N
        ...
        try
            ...
            v = verts(Kh,:); 
            in = inpolygon(xg, yg, v(:,1), v(:,2));
            g = [mean(Gx_s(in),'all','omitnan'), mean(Gy_s(in),'all','omitnan')];
    ...
    plot(ax1, P(:,1), P(:,2), 's', 'MarkerFaceColor','y', 'MarkerEdgeColor','k', 'LineWidth',1.0);
    ...
    title(ax1, sprintf('Baseline (No Sharing, No Workload)\n t = %.1f s, found %d/%d', ...
           t*dt, sum(found), M));
...
% Voronoi cells (using last C/V, still okay for visualization)
for i = 1:N
    idx = C{i}(C{i}>1); 
    if numel(idx) < 3, continue; end
    verts = V(idx,:);
...
if ~isempty(writerObj)
    try
        for i = 1:round(writerObj.FrameRate)  % about 1 second of still
            frame = getframe(f);
            writeVideo(writerObj, frame);
        end
    catch ME
...
disp(table((1:M)', found, found_time, 'VariableNames',{'person','found','time_sec'}));
...
function draw_circle(ax, c, r, fc, ec, lw)
    ...
    plot(ax, c(1)+r*cos(th), c(2)+r*sin(th), '-', 'Color', ec, 'LineWidth', lw);
    patch('XData', c(1)+r*cos(th), 'YData', c(2)+r*sin(th), ...
          'FaceColor', fc, 'EdgeColor','none', 'FaceAlpha',0.9, 'Parent', ax);
end
