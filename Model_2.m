function Model_2()
% Model 2: WITH location sharing, NO workload balancing
...
eps_dither  = 0.12;       % base noise scale
...
% ---- Setup figure ----
figName = 'Model 2: Location Sharing Only (No Workload) - Animated';
...
% ---- Setup animation recording (match Model 1 style) ----
video_filename = 'model2_seed30.avi';
try
...
for t = 1:T
    ...
    % ---- avoidance of already-found persons ----
    ...
    U_det = k_lloyd*(Ccent - P) ...
          ...
          - W ...
          - k_center*(P - center) ...
          + k_avoid_found*F_found_avoid;  % WITH location sharing
    ...
    title(ax1, sprintf('Location Sharing Only (No Workload)\n t = %.1f s, found %d/%d', ...
           t*dt, sum(found), M));

    % Right plot with full history (like Model 1)
